      ...
      <div align="center">
          {props.list.map((person) => (
              <h3 key={person.name}>
                  ...
                  />
                  <button className="smaller-custom-button" onClick={() => props.addCost(person.name)}>Add</button>
                  <button className="smaller-custom-button" onClick={() => props.onRemovePerson(person.name)}>Remove</button>
              </h3>
          ))}
      </div>
  )
  ...
  updateInputCost(name, e) {
    ...
    this.setState((currentState) => {
        if (currentState.people.length > 1) {
            ...
            return {
                people:peopleArr.slice(0,index).concat(person).concat(peopleArr.slice(index)),
            }
        } else {
            return {
                people:[person],
  ...
  handleAddCost(name) {
    ...
    if (person.items !== '') {
      ...
      this.setState((currentState) => {
        ...
        if (currentState.people.length > 1) {
            return {
                people: peopleArr.slice(0,index).concat(newPerson).concat(peopleArr.slice(index)),
                inputCost: '',
  ...
  handleRemovePerson(name) {
      this.setState((currentState) => {
        ...
        return {
          ...
          totalCostNoFees: financial(parseFloat(currentState.totalCostNoFees) - parseFloat(pers.total)),
        }
      })
  }
  updateInputTotal(e) {
      ...
      return (
          <div align="center">
            ...
            <div>
                ...
                <button className = "custombutton" onClick={this.handleAddPerson}>Submit</button>
              </div>
            <DisplayCosts 
              ...
              <div>
                <input
                    ...
                    />
                <button className = "custombutton" onClick={this.handleAddTotal}>Submit</button>
                <h2>Fees: ${parseFloat(this.state.totalCost) - parseFloat(this.state.totalCostNoFees) > 0 ? financial(parseFloat(this.state.totalCost) - parseFloat(this.state.totalCostNoFees)) : "0.00"}</h2>
